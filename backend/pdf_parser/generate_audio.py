...
from pdf_parser.dialogue_to_voice import dialogue_to_voice
from pdf_parser.make_metadata import get_transcript_metadata
...
def generate_audio():
    ...
    total_segments = sum(
        len(metadata[key]["filename"]) for key in metadata if key.startswith("segment")
    )
